...
------------------------------------------------------------
-- RACIAL UNLOCK CHECKERS
------------------------------------------------------------
local PLAYER_RACE = select(2, UnitRace("player"))

local function HasRacialUnlock(keyName)
    local race = select(2, UnitRace("player"))
    if keyName == "Mechanical" then 
        return (race == "Gnome" or race == "Goblin" or race == "Mechagnome") 
    elseif keyName == "Cloud Serpents" then 
        return (race == "Pandaren") 
    elseif keyName == "Florafaun" then
        return (race == "Haranir")
    end
    return false
end
...
local function IsUnlocked(item)
    -- 1. Check for Racial Passive first
    if HasRacialUnlock(item.keyName) then return true end

    -- 2. Check for Quests/Spells using the item's 'unlock' table
...
-- STATUS HELPERS
------------------------------------------------------------
local function StatusText(ok, isRacial)
...
local function EnsureCopyPopup()
  ...
  hint:SetText("Ctrl+C (Windows) / Cmd+C (Mac) to copy. Enter/Esc to close.")
...
local LIST = {
...

  {
    ...
    guide =
      ...
      "Use:\n" ..
      "- Buy the book from San Redscale, Serpent Keeper at The Arboretum (/way 56 44), and read it -> Cloud Serpent taming unlocked.",
  },
...

  {
    ...
    guide =
      ...
      "Note:\n" ..
      "- Haranir hunters can tame Florafaun natively.",
  },
}
...
local function EnsureUI()
  ...
  function UI.UpdateGuide()
    ...
    UI.guideSub:SetText(string.format("%s%s — %s", item.keyName, fullTag, StatusText(item.ok, item.isRacial)))
...
local isRacial = HasRacialUnlock(e.keyName)      
local ok = IsUnlocked(e)
      ...
      if not item then
        ...
        if UI.tab == "ALL" then
          ...
          text = string.format(
            ...
            item.keyName,
            expansionTag,
            StatusText(item.ok, item.isRacial)
          )
        end
